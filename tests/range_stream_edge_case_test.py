...
def test_range_integrity_check_fails(full_range_stream_fresh, error_msg):
    ...
    with raises(ValueError, match=error_msg):
        ...
        full_range_stream_fresh.check_range_integrity()


def test_range_integrity_check_pass_empty_stream_fresh(empty_range_stream_fresh):
    empty_range_stream_fresh._ranges.add(rng=Range(4, 6), value=123)
    assert empty_range_stream_fresh.check_range_integrity() is None
...
def test_subrange_register_pass_full_stream_fresh(full_range_stream_fresh):
    """
    Full RangeStream's length should be checked so calling `register_range`
    on it should be fine
    """
    full_range_stream_fresh.register_range(rng=Range(4, 6), value=123)
    assert full_range_stream_fresh.check_range_integrity() is None
...
def test_subrange(full_range_stream_fresh, start, stop, error_msg):
    with raises(ValueError, match=error_msg):
        full_range_stream_fresh.register_range(rng=Range(start, stop), value=123)


def test_nonduplicate_range_add(full_range_stream_fresh):
    """
    ...
    """
    _ = full_range_stream_fresh.read()
    full_range_stream_fresh.add(full_range_stream_fresh.total_range)
...
def test_nonduplicate_range_add_with_pruning_Head_To_Tail(
    ...
    """
    full_range_stream_fresh.pruning_level = pruning_level
    if pruning_level in range(2):
        # 0 = replant, 1 = burn
        full_range_stream_fresh.add(overlapping_range)
        assert len(full_range_stream_fresh._ranges) == expected_count
        assert len(full_range_stream_fresh.ranges) == expected_count
    else:
...
def test_nonduplicate_range_add_with_pruning_Head(
    ...
    init_active_rng = centred_range_stream_fresh._active_range
    if pruning_level in range(2):
        # 0 = replant, 1 = burn
        centred_range_stream_fresh.add(overlapping_range)
        assert len(centred_range_stream_fresh._ranges) == expected_int_count
        assert len(centred_range_stream_fresh.ranges) == expected_ext_count
        if pruning_level == 0:
            # assert one of the RangeResponse ranges values in
            # centred_range_stream_fresh.ranges == overlapping_range
            ...
            # and re-requested)
            assert centred_range_stream_fresh._active_range == overlapping_range
            rng_lst = ranges_in_reg_order(centred_range_stream_fresh._ranges)
...
def test_bad_active_range_response(full_range_stream_fresh, error_msg):
    ...
    read, may change but for now just test to clarify behaviour. See issue #4.
    """
    full_range_stream_fresh.set_active_range(123)
    with raises(ValueError, match=error_msg):
        full_range_stream_fresh.active_range_response


@mark.parametrize("error_msg", ["Cannot use total_range before setting _length"])
def test_total_range_sabotage_length(empty_range_stream_fresh, error_msg):
    """
    ...
    """
    empty_range_stream_fresh._length = None
    with raises(AttributeError, match=error_msg):
        empty_range_stream_fresh.total_range
